<template>
  <div id="mp">
    <section class="banner relative flex items-center justify-center w-screen">
      <el-image
        class="absolute w-screen h-full opacity-30 z-0"
        :src="banner"
        fit="cover"
      ></el-image>
      <div
        class="px-8 md:px-32 text-xl sm:text-4xl text-center text-white tracking-wide z-10"
      >
        <div
          class="w-1/2 mx-auto text-xl border border-solid border-gray-100 py-2"
        >
          小程序定制开发
        </div>
        <div class="pt-6">
    ...
    <section class="py-4 px-6 sm:py-12 sm:px-24 text-center">
      <div class="text-primary text-xl sm:text-4xl font-bold">
        小程序定制开发
      </div>
      <p class="w-4/5 text-left sm:text-xl mx-auto pt-6">
      ...
      <p class="w-4/5 text-left sm:text-xl mx-auto pt-1">
        <span v-html="space"></span
        >小程序是近年来最大的移动互联网商机，率先上线小程序，抢占流量红利，实现营收激增。目前，微信端小程序已支持互动直播。
      </p>
    </section>

    <section class="bg py-4 px-6 sm:py-12 sm:px-24 text-center">
      <div class="text-primary text-xl sm:text-4xl font-bold">
        小程序核心优势
      </div>
      <p class="w-4/5 text-center sm:text-xl mx-auto pt-2">
        丰富的使用场景，离用户更近一步
      </p>
      <div class="flex flex-wrap items-center justify-around pt-6 sm:pt-10">
        <el-card
          class="flow items-center flow-col pt-6 px-2 w-40 sm:w-48 h-80 mb-6"
        ...
        >
          <el-image
            ...
            fit="fill"
          ></el-image>
          <div class="font-bold text-center text-gray-800 py-3">
            {{ advantage.title }}
          </div>
          <div class="text-md text-center">{{ advantage.desc }}</div>
        </el-card>
      </div>
    </section>

    <section class="bg py-4 px-6 sm:py-12 sm:px-24 text-center">
      <div class="text-primary text-xl sm:text-4xl font-bold">行业解决方案</div>
      <p class="w-4/5 text-center sm:text-xl mx-auto pt-2">
        定制化行业需求，打造专属产品
      </p>
      <div class="py-8 flex flex-wrap justify-around">
        <div
          v-for="(solution, index) in solutionList"
          :key="index"
          class="w-40 md:w-64 h-40 md:h-64 p-1 flex items-center justify-center"
          ...
          >
            <div slot="reference">
              ...
              <el-image
                ...
                fit="cover"
              ></el-image>
              <div class="absolute inset-0 z-0 bg-opacity-60 bg-black"></div>
              <div
                class="absolute inset-0 z-0 bg-opacity-40 delay-100 duration-500 transition-transform transform bg-green-600"
                :class="
                  shadow == index + 1 ? 'translate-y-0' : 'translate-y-full'
                "
              ></div>
              <el-image
                class="w-14 h-16 md:w-20 md:h-24 z-10 -mt-6 flex-shrink-0"
                :src="solution.icon"
                fit="fill"
              ></el-image>
              <div
                class="absolute text-xl md:text-2xl font-bold text-center pt-3 text-white z-10 inset-x-0"
              >
                {{ solution.label }}
              </div>
            </div>
    ...
    <section class="bg py-8 flex items-center flex-wrap">
      <div
        class="w-1/2 text-left pl-6 sm:w-2/5 sm:text-center sm:pl-0 text-2xl sm:text-4xl text-black py-10"
      >
        开发资讯
      </div>
      <div class="flex-1 text-left px-6 sm:pr-10 sm:pl-0">
        ...
        <div class="post-group">
          <el-card
            class="mt-6 bg cursor-pointer border-none"
            v-for="(post, i) in postList"
            :key="i"
            shadow="hover"
            @click.native="tapToPost(i)"
          >
            <div class="flex">
              <el-image
                class="w-16 h-16"
                :src="
                  post.thumbnail
                    ? require('@/assets/post/' + post.thumbnail)
                    : logoFinal
                "
                fit="fill"
              ></el-image>
              <div class="flex-1 pl-6 pr-0 sm:pl-8 sm:pr-8">
                <div
                  class="text-xl sm:text-2xl text-black font-bold"
                  to="/post"
                >
                  {{ post.title }}
                </div>
      ...
      solutionList: [
        {
          ...
          label: "电商",
          qrcode: require("@/assets/qrcode/mp-solution-qrcode-1.png")
        },
        {
        ...
        },
        {
          ...
          label: "餐饮",
          qrcode: require("@/assets/qrcode/mp-solution-qrcode-3.png")
        },
        {
